...
class RAGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            pos_ctxs = inputs.get("positive_ctxs", [])

            if len(pos_ctxs) == 0:          # 없다면 빈 passage
                title, passage = "", ""
            else:
                ctx = random.choice(pos_ctxs)
                title = ctx.get("title", "")
            ...
            # 컨텍스트 문자열 (타이틀 + 패시지)
            input_text_ctx = (
                f"{self.prefix_title}{title}\n"
                f"{self.prefix_passage}{passage}"
            ).strip()

            # 정답(answers[0]) 을 제외한 입력부분
